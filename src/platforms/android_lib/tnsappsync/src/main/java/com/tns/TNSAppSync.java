...
public class TNSAppSync {
    ...
    private static String getCurrentPackagePath(final Context context) {
        ...
        final String appSyncAppVersion = getCurrentAppVersion(context);
        final String appSyncAppBuildTime = getCurrentAppBuildTime(context);

        if (appStoreAppVersion == null || appStoreAppBuildTime == null || appSyncAppBuildTime == null || appSyncAppVersion == null) {
            // this shouldn't happen, let's revert back to the appstore version
            removeCurrentHash(context);
            return null;
        }

        final boolean appSyncPackageIsNewerThanAppStoreVersion =
                appSyncAppBuildTime.equals(appStoreAppBuildTime) && appSyncAppVersion.equals(appStoreAppVersion);
    ...
    private static void removePendingHash(final Context context) {
        getPreferences(context)
                .edit()
                .remove(APPSYNC_PENDING_HASH_KEY)
                .apply();
    }
